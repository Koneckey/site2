...
def index():
    if request.method == 'POST':
        if request.form.get('login'):
            ...
            if errors != "" :
                return errors 
            conn = psycopg2.connect(database = "int",
                                user="postgres",
                                password="1234",
                                host="localhost",
                                port="5432")
            cursor = conn.cursor()
            cursor.execute("SELECT name FROM player WHERE login=%s and password=%s",
...
def index2():
    if request.method == 'POST':
        ...
        conn = psycopg2.connect(database = "int",
                                ...
                                port="5432")
        cursor = conn.cursor()
        cursor.execute("INSERT INTO player (name,login,password) VALUES (%s,%s,%s );",
